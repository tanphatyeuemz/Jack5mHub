...
function DisbaleNoClip()
    if plr.Character.HumanoidRootPart:FindFirstChild("EffectsSY") then
        plr.Character.HumanoidRootPart:FindFirstChild("EffectsSY"):Destroy()
    end
end
...
CheckDoorPlates = function()
    return workspace.Map.Jungle.QuestPlates.Door.CanCollide
end
GetPlates = function()
    for r, v in pairs(workspace.Map.Jungle.QuestPlates:GetChildren()) do
...
SaberDoorOpened = function()
    return workspace.Map.Jungle.Final.Part.CanCollide
end
CheckDesertdDoor = function()
    return workspace.Map.Desert.Burn.Fire.CanCollide
end
...
spawn(function()
    while wait() do
        if getgenv().AutoSaber then
            if not CheckBackpack("Saber") and game.Players.LocalPlayer.Data.Level.Value >= 200 then
                if SaberDoorOpened() then
                    if CheckDoorPlates() then
                        pcall(function()
                            Tween(GetPlates())
                        end)
                    else
                        if CheckDesertdDoor() then
                            if not CheckBackpack("Torch") then
                                local Torch = workspace.Map.Jungle.Torch
                                Tween(Torch.CFrame)
                            else
                                EquipTool("Torch")
                                Tween(CFrame.new(1115.25354, 4.95647621, 4349.24463, -0.624247193, 5.46374288e-08, 0.781226873, -1.33858959e-08, 1, -8.06341163e-08, -0.781226873, -6.07930417e-08, -0.624247193))
                            end
                        else
                            local RichsonProgess = game.ReplicatedStorage.Remotes.CommF_:InvokeServer("ProQuestProgress", "RichSon")
                            if RichsonProgess ~= 0 and RichsonProgess ~= 1 then
                                if not CheckBackpack("Cup")  then
                                    ...
                                    EquipTool("Cup")
                                    wait(0.4)
                                    game.ReplicatedStorage.Remotes.CommF_:InvokeServer("ProQuestProgress","FillCup", CheckBackpack("Cup"))
                                    wait(0.2)
                                    game.ReplicatedStorage.Remotes.CommF_:InvokeServer("ProQuestProgress","SickMan")
                                end
                            elseif RichsonProgess == 0 then
                                if CheckMob("Mob Leader", true) then
                                    local v = CheckMob("Mob Leader", true)
                                    if IsAlive(v) and v.Parent then
                                        repeat wait()
                                            EnableBuso()
                                            EquipTool(GetWeapon())
                                            Tween(v.HumanoidRootPart.CFrame * CFrame.new(0, 20, 0))
                                        until not v or not IsAlive(v) or getgenv().AutoSaber == false
                                    end
                                else
                                    Tween(CFrame.new(-2836.19385, 7.92910671, 5484.93408))
                                end
                            elseif RichsonProgess == 1 then
                                if not CheckBackpack("Relic") then
                                    local Rich = CFrame.new(-908.814819, 14.1188612, 4077.24658)
                                    if Getdistance(Rich.Position) > 10 then
                                        Tween(Rich)
                                    else
                                    ...
                                    EquipTool("Relic")
                                    Tween(CFrame.new(-1404.43835, 29.8870087, 4.62949038, 0.883046508, 9.38910727e-09, 0.469285458, -2.02383887e-08, 1, 1.80749922e-08, -0.469285458, -2.54586396e-08, 0.883046508))
                                end
                            end
                        end
                    end
                    ...
                    if CheckMob("Saber Expert", true) then
                        ...
                        repeat wait()
                            ...
                            EquipTool(GetWeapon())
                            Tween(v.HumanoidRootPart.CFrame * CFrame.new(0, 20, 0))
                        until not IsAlive(v) or not v.Parent or getgenv().AutoSaber == false
                    end
                end
            end
        end
    ...
    while task.wait() do
        if getgenv().TPFruit then
            if GetFruitSpawn() then
                Tween(GetFruitSpawn().Handle.CFrame)
            end
        end
    end
end)
